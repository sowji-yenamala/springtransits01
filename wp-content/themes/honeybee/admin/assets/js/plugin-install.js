jQuery(document).ready(function($) {
    'use strict';
    var this_obj = honeybee_companion_install;
    ...
    $(document).on('click', '.action-watch', function(event) {
        ...
        if(action_id){
            $.ajax({
                ...
                type: 'POST',
                data: {action: 'honeybee_update_rec_acts', action_id:action_id},
                success:function(data) {
    ...
    $(document).on('click', '.spicethemes-customizer-notification-dismiss', function(event) {
        ...
        var sdata = {};
        if(slug){
            sdata.action = 'honeybee_hide_customizer_notice';
            sdata.honeybee_plugin_slug = slug;
        }else{
            sdata.action = 'honeybee_hide_customizer_companion_notice';
        }
